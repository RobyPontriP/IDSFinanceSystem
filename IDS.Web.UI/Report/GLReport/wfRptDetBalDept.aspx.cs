...
{
    public partial class wfRptDetBalDept : System.Web.UI.Page
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                rpt.Load(Server.MapPath(@"~/Report/GLReport/CR/RptDetailTrialBalanceDept.rpt"));
                rptHelper.SetDefaultFormulaField(rpt);
                rptHelper.SetLogOn(rpt);

                rpt.SetParameterValue("@branchcode", Request.Params["ctl00$ContentPlaceHolder1$cboBranch"]);
                rpt.SetParameterValue("@pDept", Request.Params["ctl00$ContentPlaceHolder1$cboKarep"]);
                rpt.SetParameterValue("@pTxtPrd1", string.IsNullOrEmpty(Request.Params["ctl00$ContentPlaceHolder1$txtDtpPeriodFrom"]) ? DateTime.Now.Date : Convert.ToDateTime(Request.Params["ctl00$ContentPlaceHolder1$txtDtpPeriodFrom"]));
                rpt.SetParameterValue("@pTxtPrd2", string.IsNullOrEmpty(Request.Params["ctl00$ContentPlaceHolder1$txtDtpPeriodTo"]) ? DateTime.Now.Date : Convert.ToDateTime(Request.Params["ctl00$ContentPlaceHolder1$txtDtpPeriodTo"]));
                rpt.SetParameterValue("@pAccFrom", DBNull.Value);
                rpt.SetParameterValue("@pAccTo", DBNull.Value);
                rpt.SetParameterValue("@pCcyFrom", DBNull.Value);
                rpt.SetParameterValue("@pCcyTo", DBNull.Value);
            ...
            else
            {
                ...
                rpt.SetParameterValue("@pTxtPrd1", string.IsNullOrEmpty(Request.Params["ctl00$ContentPlaceHolder1$txtDtpPeriodFrom"]) ? DateTime.Now.Date : Convert.ToDateTime(Request.Params["ctl00$ContentPlaceHolder1$txtDtpPeriodFrom"]));
                rpt.SetParameterValue("@pTxtPrd2", string.IsNullOrEmpty(Request.Params["ctl00$ContentPlaceHolder1$txtDtpPeriodTo"]) ? DateTime.Now.Date : Convert.ToDateTime(Request.Params["ctl00$ContentPlaceHolder1$txtDtpPeriodTo"]));
                rpt.SetParameterValue("@pAccFrom", string.IsNullOrEmpty(Request.Params["ctl00$ContentPlaceHolder1$cboAccFrom"]) ? "" : Request.Params["ctl00$ContentPlaceHolder1$cboAccFrom"]);
                rpt.SetParameterValue("@pAccTo", string.IsNullOrEmpty(Request.Params["ctl00$ContentPlaceHolder1$cboAccTo"]) ? "" : Request.Params["ctl00$ContentPlaceHolder1$cboAccTo"]);
                rpt.SetParameterValue("@pCcyFrom", string.IsNullOrEmpty(Request.Params["ctl00$ContentPlaceHolder1$cboCcyFrom"]) ? "" : Request.Params["ctl00$ContentPlaceHolder1$cboCcyFrom"]);
                rpt.SetParameterValue("@pCcyTo", string.IsNullOrEmpty(Request.Params["ctl00$ContentPlaceHolder1$cboCcyTo"]) ? "" : Request.Params["ctl00$ContentPlaceHolder1$cboCcyTo"]);
        ...
        {
            cboKarep.DataSource = IDS.GeneralTable.Department.GetDepartmentForDataSource(branchCode);
            cboKarep.DataTextField = "Value";
            cboKarep.DataValueField = "Value";
            cboKarep.DataBind();
            cboKarep.SelectedValue = Request.Params["ctl00$ContentPlaceHolder1$cboKarep"];
        }
